...
local cfg;
local TT_DefaultConfig = { -- DaMaGepy
	showUnitTip = true,
	showStatus = true,
	showGuildRank = false,
	showTargetedBy = true,
...
function tt:ApplySettings()
	...
	--if (cfg.hideWorldTips) then self:RegisterEvent("CURSOR_UPDATE"); else self:UnregisterEvent("CURSOR_UPDATE"); end -- DaMaGepy
...
function tt:RemoveUnwantedLines(tip) -- DaMaGepy
...
function tt:AddTargetedBy(u) -- DaMaGepy
	...
	if targetedByList then if targetedByList.count then
...
gtt.FadeOut = function(self,...)
    ...
    if self.ttFadeOut ~= FADE_BLOCK then
