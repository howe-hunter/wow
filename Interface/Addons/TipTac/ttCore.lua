...
local TT_DefaultConfig = {
	...
	tipBackdropBG = "Interface\\FrameGeneral\\UI-Background-Rock",
	...
	fontFace = "Fonts\\FRIZQT__.TTF",	-- Set during VARIABLES_LOADED
	fontSize = 12,
	fontFlags = "",
	...
	talentFormat = 1,
	talentCacheSize = 25,
	inspectDelay = 0.5,			-- The time delay for the scheduled inspection
...
function tt:ApplyTipBackdrop(tip)
	if not tipBackdrop then tipBackdrop=TOOLTIP_BACKDROP_STYLE_DEFAULT; end;
	if SharedTooltip_SetBackdropStyle then SharedTooltip_SetBackdropStyle(tip,tipBackdrop); -- BCC
	elseif GameTooltip_SetBackdropStyle then GameTooltip_SetBackdropStyle(tip,tipBackdrop); end; -- ERA
end
...
function tt:RemoveUnwantedLines(tip)
	for i = 2, tip:NumLines() do
		...
		if text~=nil then if (strsub(text,-1)==">") and strfind(text," / ")~=nil then line:SetText(nil); end; end; -- < ... / ... >
		...
		if text~=nil then if (strsub(text,-1)==")") or (strsub(text,1,6)=="Level ") then line:SetText(nil); end; end;
	end
end
...
function tt:AddTargetedBy(u)
	...
	for i = 1, numGroup do
		...
		if (UnitIsUnit(unit.."target",u.token)) and (not UnitIsUnit(unit,"player")) then
			...
			local FUnitName = UnitName(unit);
			if GetFakeName~=nil then FUnitName=GetFakeName(FUnitName,"TipTacStyle:GenPlayerLines") end -- DaMaGepy
			targetedByList.next = FUnitName;
...
function gttScriptHooks:OnUpdate(elapsed)
	...
	if SharedTooltip_SetBackdropStyle then SharedTooltip_SetBackdropStyle(self, tipBackdrop); -- BCC
	elseif GameTooltip_SetBackdropStyle then GameTooltip_SetBackdropStyle(self, tipBackdrop); end; -- ERA
end
