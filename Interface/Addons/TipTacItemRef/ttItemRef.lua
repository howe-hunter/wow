...
local function ttSetIconTextureAndText(self, texture, count)
	if (texture) then
		...
		self.ttIcon:Hide();
		if self.ttCount then self.ttCount:SetText(""); end
	end
end
...
function ttif:CreateTooltipIcon(tip)
	...
	if (cfg.if_iconAnchor~=nil) and (cfg.if_iconTooltipAnchor~=nil) and (cfg.if_iconOffsetX~=nil) and (cfg.if_iconOffsetY~=nil) then -- DaMaGepy debug
	...
	for index, tip in ipairs(tipsToModify) do
		if (type(tip) == "table") and (tipsToAddIcon[tip:GetName()]) and (tip.ttIcon) then
			if (cfg.if_showIcon) then
				...
				if tip.ttCount then tip.ttCount:SetFont(gameFont, (cfg.if_iconSize / 3), "OUTLINE"); end -- DaMaGepy
	...
	if (cfg.if_enable) and (not tipDataAdded[self]) then
		...
		if (spellID) then
			...
			if (_name ~= "" and PetHasSpellbook()) then -- id is missing. as a workaround find pet action in pet spell book by name.
				...
				if numPetSpells then -- DaMaGepy fix
				for i = 1, numPetSpells do
...
function ttif:ADDON_LOADED(event, addOnName)
	...
	elseif (addOnName == "Blizzard_Communities") or ((addOnName == "TipTacItemRef") and (IsAddOnLoaded("Blizzard_Communities"))) then
		--hooksecurefunc("CommunitiesGuildNewsButton_OnEnter", GNB_OnEnter_Hook); -- DaMaGepy

		-- Function to apply necessary hooks to CommunitiesFrameGuildDetailsFrameInfo
		ttif:ApplyHooksToCFGDFI();
		
		--hooksecurefunc("CommunitiesGuildInfoFrame_UpdateChallenges", function() ttif:ApplyHooksToCFGDFI(); end); -- DaMaGepy
